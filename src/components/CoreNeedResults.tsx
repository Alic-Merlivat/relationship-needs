...
import type { RankedNeedView } from "@/lib/ranking";
import { CATEGORY_ACCENT, CATEGORY_GRADIENT } from "@/lib/theme";

const TOP_CORE_NEEDS_SHOWN = 3;

/**
 * The Core-Need-first results view.
 *
 * Takes a raw history rather than reading storage so the same component
 * renders both the just-finished local result and a saved one loaded from
 * the server by token.
 */
export function CoreNeedResults({ history }: { history: ComparisonRecord[] }) {
  const coreNeeds = useMemo(() => buildCoreNeedRanking(history), [history]);
